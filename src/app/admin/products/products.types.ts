...
export const productSchema = z.object({
    ...
    images: z
        ...
        .refine(
            (files) => {
                if (!files || files.length === 0) return true; // Bypass check if optional field is empty
                return Array.from(files).every((file: any) => file.size <= 1024 * 1024);
            },
            { message: 'Every image must be less than 1MB.' }
        )
        // 3. Check the MIME type for EVERY file in the list (if any exist)
        .refine(
            (files) => {
                if (!files || files.length === 0) return true; // Bypass check if optional field is empty
