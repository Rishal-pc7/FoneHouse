...
export default function AddProductForm() {
    ...
    const [mainImg, setMainImg] = useState<File | undefined | Blob>(undefined);
    ...
    const onSubmit = async (data: ProductFormData) => {
        try {
            ...
            if (validatedData.img?.[0]) {
                formData.append('img', validatedData.img[0]);
            }

            if (validatedData.images && validatedData.images.length > 0) {
                Array.from(validatedData.images).forEach((file: any) => {
                    formData.append('images', file);
                });
            }

            const result = await fetch("/api/admin/addProduct", {
                method: "POST",
                body: formData,
            });
    ...
    useEffect(() => {
        const newAdditionalPreviews = additionalImages.map(f => URL.createObjectURL(f))
        setAdditionalPreviews(newAdditionalPreviews)
        return () => {
            newAdditionalPreviews.forEach(image => URL.revokeObjectURL(image))
        }
    }, [additionalImages])
    useEffect(() => {
        if (!mainImg) return
                ...
                {(() => {
                    ...
                    return (
                        <div className="space-y-6 bg-gray-50 dark:bg-zinc-800/50 p-6 rounded-2xl border border-gray-100 dark:border-zinc-800">
                            ...
                            <div>
                                ...
                                {!mainImage ? (
                                    ...
                                    <div className="relative w-40 h-40 rounded-xl border border-gray-200 dark:border-zinc-700 group overflow-hidden bg-white dark:bg-zinc-900 shadow-sm">
                                        <img src={mainImgPreview || undefined} alt="Main" className="object-cover w-full h-full" />
                                        <button
                                            type="button"
                                            onClick={() => {
                                ...
                                <label className="flex flex-col items-center justify-center w-full h-32 px-4 py-3 mb-4 rounded-xl border-2 border-dashed border-gray-300 dark:border-zinc-700 hover:border-brandBlue hover:bg-white dark:hover:bg-zinc-800 cursor-pointer transition-all">
                                    ...
                                    <input
                                        ...
                                        onChange={(e) => {
                                            ...
                                            if (!files) return;
                                            let newImages: File[] = [...additionalImages];
                                            ...
                                            <div key={i} className="relative aspect-square w-full rounded-xl border border-gray-200 dark:border-zinc-700 group overflow-hidden bg-white dark:bg-zinc-900 shadow-sm">
                                                ...
                                                <button
                                                    ...
                                                    onClick={() => {
                                                        ...
                                                        const oldValue = watch('images') || [];
                                                        const newValue = oldValue.length > 0 ? Array.from(oldValue).filter((_, idx) => idx !== i) : [];
                                                        setValue('images', newValue, { shouldValidate: true, shouldDirty: true });
                                                    }}
                                                    className="absolute top-2 right-2 bg-red-500/90 hover:bg-red-500 text-white p-2 rounded-lg opacity-0 group-hover:opacity-100 transition-all shadow-sm backdrop-blur-sm"
                                                >
                                                    <Trash size={14} />
                            ...
                            {errors.images && <p className="text-red-500 text-sm mt-2">{errors.images?.message as string}</p>}
                        </div>
                    );
                })()}
            </div>
